...
const ManaCostButton = ({ selectedCost, cost, costClear, selectedFilter }) => {
  ...
  useEffect(() => {
    selectedFilter.cost.includes(cost) && setIsBorderActive(true);
  }, []);
  ...
  return (
    <div>
      <button
        className={
          isBorderActive
            ? 'btn btn-circle btn-sm bg-error text-slate-200 border-2 border-yellow-500'
            : 'btn btn-circle btn-sm bg-error text-slate-200'
        }
